...
def OnQuote(context, code) :
    ...
    if len(klinedata) > 1 and klinedata[-1].close < klinedata[-2].close * 0.99 and context.myacc :
        # 当日收盘价小于昨收的0.99 则买入 100手
        context.myacc.InsertOrder(code, BSType.Buy, dyndata.now, 100)
    elif len(klinedata) > 1 and klinedata[-1].close > klinedata[-2].close * 1.01 and context.myacc :
        # 当日收盘价大于昨收的1.01倍 则卖出 100手
        context.myacc.InsertOrder(code, BSType.Sell, dyndata.now, 100)
    elif len(klinedata) > 1 and klinedata[-1].close < klinedata[-2].close * 0.98 and context.myacc :
        # 当日收盘价小于昨收的0.98 则卖出 100手  止损
        context.myacc.InsertOrder(code, BSType.Sell, dyndata.now, 100)
